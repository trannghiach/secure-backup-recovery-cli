...
class StorageManager:
    def __init__(self, store_path: str = "store"):
        self.base_path = (Path(os.getcwd()) / store_path).resolve()
    ...
    def load_manifest(self, snapshot_id: str) -> SnapshotManifest:
        # --- [FIX] Trả về Object SnapshotManifest ---
        ...
        # Convert List Dict -> List FileEntry Objects
        ...
        return SnapshotManifest(
            ...
            prev_snapshot_id=data.get('prev_snapshot_id'),
            files=files
        )
    ...
    def restore(self, snapshot_id: str, restore_path: str) -> OpResult:
        ...
        try:
            ...
            return OpResult(success=False, message=f"Snapshot error: {e}")

        # 2. Verify (Object)
        print("Verifying snapshot integrity...")
        if not self._verify_snapshot_integrity(manifest):
            return OpResult(success=False, message="Integrity Check Failed: Merkle root mismatch.")
        ...
        restored_count = 0
        for f in manifest.files: # Dùng object attribute
            rel_path = f.path
            chunks_hashes = f.chunks
            mtime = f.mtime
            ...
            try:
                with open(dest_path, 'wb') as f_out:
                    for ch in chunks_hashes:
                        try:
                            f_out.write(self.get_chunk(ch))
                        except FileNotFoundError:
                            f_out.close()
                            if dest_path.exists(): os.remove(dest_path)
                            return OpResult(success=False, message=f"Missing chunk {ch}")
                
                if mtime: os.utime(dest_path, (time.time(), mtime))
                restored_count += 1
            except OSError as e:
                return OpResult(success=False, message=f"I/O Error: {e}")

        return OpResult(success=True, message=f"Restored {restored_count} files.")